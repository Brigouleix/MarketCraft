...
class SearchController extends Controller
{
    ...
    // ------------------------------------------------------------------
    // GET /search/recommendations/:productId  (Option C CDC)
    // ------------------------------------------------------------------
    public function recommendations(array $params = []): void
    {
        $productId = (int) ($params['productId'] ?? $params['id'] ?? 0);
        ...
        $stmt = $pdo->prepare(
            'SELECT p.*, b.nom AS boutique_nom
             FROM produits p
        ...
        $productContext = sprintf(
            'Produit : %s. Catégorie : %s. Tags : %s. Description : %s',
            $sourceProduct['nom'],
            $sourceProduct['categorie'] ?? 'Non définie',
            $sourceProduct['tags'] ?? 'aucun',
            substr($sourceProduct['description'] ?? '', 0, 200)
        );

        $prompt = "Tu es un expert en recommandations produits pour une marketplace artisanale.
Voici un produit : {$productContext}

Génère UNIQUEMENT un objet JSON avec :
1. \"keywords\" : tableau de 4-6 mots-clés pour trouver des produits similaires ou complémentaires
2. \"reason\" : phrase courte (max 15 mots) expliquant la logique de recommandation

Exemple : {\"keywords\":[\"bois\",\"chêne\",\"table\",\"naturel\"],\"reason\":\"Produits en bois naturel similaires pour votre intérieur.\"}

Réponds UNIQUEMENT avec le JSON.";

        $aiResponse = $this->callClaudeApi($prompt);
        $aiReason   = 'Produits similaires sélectionnés pour vous.';
        $keywords   = [];

        if ($aiResponse !== null) {
            $keywords = $aiResponse['keywords'] ?? [];
            $aiReason = $aiResponse['reason']   ?? $aiReason;
        }

        // Fallback : utiliser les tags du produit source
        if (empty($keywords) && !empty($sourceProduct['tags'])) {
            $keywords = array_slice(explode(',', $sourceProduct['tags']), 0, 4);
        }

        // Chercher les produits recommandés (exclure le produit courant)
        $recommendations = $this->searchProducts($keywords);
        $recommendations = array_filter($recommendations, fn($p) => $p['id'] !== $productId);
        $recommendations = array_values(array_slice($recommendations, 0, 4));
